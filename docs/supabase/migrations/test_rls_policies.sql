...
BEGIN;
  ...
  UPDATE profiles SET language = 'en'
    WHERE firebase_uid = 'test-customer-uid'; -- Expected: 1 row updated
ROLLBACK;
...
BEGIN;
  SET LOCAL role authenticated;
  SET LOCAL request.jwt.claims = '{"sub": "test-driver-uid"}';
  SELECT count(*) AS other_bids             -- Expected: 0
...
-- ############################################################################
-- DOCUMENTS (table name in schema: `documents`)
-- ############################################################################

-- Test 12: User can view their own documents
BEGIN;
  SET LOCAL role authenticated;
  SET LOCAL request.jwt.claims = '{"sub": "test-driver-uid"}';
...
-- Test 13: User cannot view another user's documents
BEGIN;
  SET LOCAL role authenticated;
  SET LOCAL request.jwt.claims = '{"sub": "test-driver-uid"}';
